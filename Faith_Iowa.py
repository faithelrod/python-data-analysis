...
def _():
    ...
    data_file = 'Iowa_Liquor_Sales-26M.csv'
...
def _(data_file, pl):
    orig_df = pl.read_csv(
        ...
        schema_overrides={
            "Zip Code": pl.Utf8,
            "Store Number": pl.Utf8, 
            "Vendor Number": pl.Utf8, 
            "Item Number": pl.Utf8
        },
        infer_schema_length=2000
    )

    size = orig_df.estimated_size()
    size_mb = size / (1024 ** 2)
...
def _(mo):
    ...
    |---|---|---|---|
    | Date  | String (Utf8) | Date | converts string dates to native dates for effient filtering  |
    | Store Number  | String (Utf8) | Int32 | reduced memory adn improves performance |
    | Vendor Number  | String (Utf8) | Int32 | reduced memory |
    | Item Number  | String (Utf8) | Int32 | converts valid numbers to Int32 |
    | Bottles Sold  | Int64) | Int32 | saves memory |
    | Sale (Dollars)  | Float64 | Float32 | reduces memory by 50% |
    | State Bottle Cost  | Float64 | Float32 | reduces memory by 50% |
    | State Bottle Retail  | Float64 | Float32 | reduces memory by 50% |
    | Volume Sold (Liters)  | Float64 | Float32 | reduces memory by 50% |
    ...
    |---|---|---|---|
    | Year | Int32 | enables year by year analysis | extracted from date column using dt.year() |
    | Month | Int32 | identifies monthly trends | extracted from Date column using dt.month() |
    | Quarter | Int32 | quarterly trends | extracted from Date using dt.quarter() |
    | DayOfWeek | Int32 | day-of-week patterns | extracted from Date using dt.weekday() |
    | IsWeekend | Boolean | compare weekend vs weekday patterns | DayOfWeek >= 5 |

    ### Other changes to the data

    Create a bulleted list for any other changes you made to the DataFrame.

    ### Rationale for Optimization

    Discuss the benefits or reasons for your changes in a bulleted list:
    """
    )
    return
...
def _(orig_df, pl):
    df = (
        ...
        .filter(
            ...
            (pl.col("Bottle Sold") > 0) &
            (pl.col("Item Number").is_not_null())
        )
    )
    return
    ...
    df_categorized = df.with_columns(
        ...
        .when(
            pl.col(\"Category Name\").str.to_uppercase().str.contains(
                \"AMARETTO|CORDIALS|LIQUERS|ANISEETE|CREME DE|TRIPLE SEC\"
            )
        )
        .then(pl.lit(\"LIQUERS & CORDIALS\"))
        .when (pl.col(\"Category Name\").str.to_uppercase().str.contains(\"AMERICAN ALCOHOL|AMERICAN DISTILLED SPIRITS SPECIALTY|DISTILLED SPIRITS SPECIALTY|IMPORTED DISTILLED SPIRITS SPECIALTY|NEUTRAL GRAIN SPIRITS\"))
        .then(pl.lit(\"SPECIALTY & OTHER SPIRITS\"))
        .when (pl.col(\"Category Name\").str.to_uppercase().str.contains(\"AMERICAN COCKTAILS|COCKTAILS|RTD\"))
        ...
        .when (pl.col(\"Category Name\").str.to_uppercase().str.contains(\"DECANTERS|SPECIALTY PACKAGES|DELISTED|SPECIAL ORDER|HIGH PROOF BEER|HOLIDAY VAP|TEMPORARY\"))
        .then(pl.lit(\"ADMINISTRATIVE/ NON-PRODUCT\"))
        .otherwise(pl.lit(\"UNCATEFGORIZED\"))
...
def _(df_categorized, mo, pl, px):
    quarterly_sales = (
        ...
        pl.col("Sale (Dollars)").sum().alias("Total_Revenue")
        ]).sort(["Year", "Quarter"])
        .with_columns([
...
def _(df_categorized, mo, pl, px):
    cities_efficiency = (
        ...
        .with_columns([
            (pl.col("Total Revenue") / pl.col("Transactions")).alias("Avg Sales per Transaction")
        ])
